...
@RestController
public class GetInvoiceInfoController {
    @Autowired
    private GetInvoiceInfoService getInvoiceInfoService;

    @PostMapping("/finance/getInvoiceInfo/")
    public JSONObject caiwu(@RequestBody Map<String, String> map){
        Integer invoice_id = Integer.parseInt(map.get("invoice_id"));
        return getInvoiceInfoService.getInvoiceInfo(invoice_id);
    }
}
